...
const Output: React.FC<OutputProps> = ({ aiOutput, codeOutput, isLoading, isExecuting, error, testCode, testResults, onRunTests, bugs, onQuickFix, language }) => {
  ...
  const renderAiContent = () => {
    ...
    if (bugs.length > 0) {
      return (
        <div>
          <h3 className="text-lg font-bold mb-3 text-gray-200">Bugs Found</h3>
          <ul className="space-y-3">
            {bugs.map((bug, index) => (
              <li key={index} className="flex items-start justify-between p-3 rounded-md bg-gray-800 border border-gray-700 shadow-sm">
                <div className="flex-1 mr-4">
                  <div className="flex items-center text-sm mb-1">
                    <span className={`font-bold mr-2 ${
                      bug.severity === 'error' ? 'text-red-400' : 
                      bug.severity === 'warning' ? 'text-yellow-400' : 'text-blue-400'
                    }`}>
                      [{bug.severity.toUpperCase()}]
                    </span>
                    <span className="text-gray-400 font-mono">L{bug.line}:{bug.column}</span>
                  </div>
    ...
    return (
      <>
        {testCode && (
           <div className="mb-4 p-3 bg-gray-800 rounded-lg border border-gray-700">
              <button 
                  ...
                  className="w-full bg-yellow-600 text-white font-bold py-2 px-4 rounded-md hover:bg-yellow-700 transition-colors duration-200 flex items-center justify-center space-x-2 disabled:bg-gray-600 disabled:cursor-not-allowed"
              >
        ...
        <div className="prose prose-invert max-w-none prose-pre:bg-gray-800 prose-pre:rounded-lg prose-headings:text-blue-300 prose-a:text-blue-400 hover:prose-a:text-blue-500">
          <ReactMarkdown
      ...
      <div className="flex-shrink-0 bg-gray-800 border-b border-gray-700">
        <nav className="flex space-x-1" aria-label="Tabs">
          <button
            ...
            className={`px-4 py-2 text-sm font-medium rounded-t-lg ${activeTab === 'ai' ? 'bg-gray-900 text-white' : 'text-gray-400 hover:text-white hover:bg-gray-700'}`}
          >
            AI Analysis
          </button>
          <button
            onClick={() => setActiveTab('code')}
            className={`px-4 py-2 text-sm font-medium rounded-t-lg ${activeTab === 'code' ? 'bg-gray-900 text-white' : 'text-gray-400 hover:text-white hover:bg-gray-700'}`}
          >
            Code Output
          </button>
        </nav>
